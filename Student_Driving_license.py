...
label_iden = Label(root)
label_name = Label(root)
label_dob = Label(root)
label_pin = Label(root)
label_adres = Label(root)
label_adfv = Label(root)

# Define the function
def myCardDetails():
    name = "Hridansh Vijay Hirpara"
    print(type(name))
    iden = 19827198349
    print(type(iden))
    dob = "25 March 2010"
    ...
    label_iden["text"] = iden
    label_dob["text"] = dob
    label_pin["text"] = pin
    label_adres["text"] = adres
    label_adfv["text"] = adfv
    
    
# Create a button
button1 = Button(root, text = "Show my Driver Id", command=myCardDetails)
